...
Q3
SELECT StudentRegistrationsToDegrees.DegreeId, count(case when gender='F'then 1.0 end)/cast (count(gender)AS FLOAT)*100.0 AS Percentage FROM (SELECT studentRegistrationsToDegrees.StudentregistrationId, Degrees.DegreeId FROM Degrees, StudentRegistrationsToDegrees, courses, CourseRegistrations, CourseOffers WHERE Degrees.DegreeId = StudentRegistrationsToDegrees.DegreeId AND Courses.DegreeId = Degrees.DegreeId AND StudentRegistrationsToDegrees.StudentRegistrationId = CourseRegistrations.StudentRegistrationId AND CourseRegistrations.CourseOfferId = CourseOffers.CourseOfferId AND Courses.CourseId = CourseOffers.CourseId AND grade>=5 GROUP BY StudentRegistrationsToDegrees.StudentRegistrationId, Degrees.DegreeId HAVING sum(ECTS) < TotalESCTS) AS Passed, students, StudentRegistrationsToDegrees, CourseRegistrations WHERE  (StudentRegistrationsToDegrees.StudentRegistrationId = Passed.StudentRegistrationId OR (StudentRegistrationsToDegrees.StudentRegistrationId NOT IN (select CourseRegistrations.StudentRegistrationId WHERE Grade>=5))) AND Students.studentId = StudentRegistrationsToDegrees.StudentId GROUP BY StudentRegistrationsToDegrees.DegreeId ORDER BY DegreeId;
